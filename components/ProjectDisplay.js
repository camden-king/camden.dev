...
export default function ProjectDisplay({ project }) {
  ...
  return (
    <Link href={`/projects/${project.id}`}>
        <a className="w-60 h-60 sm:w-80 sm:h-80 m-1 drop-shadow-xl rounded-xl relative group border-2 border-transparent text-white">
            ...
            <div className='bg-gradient-to-t from-black/75 via-black/0 h-full rounded-xl flex flex-col justify-end p-3 group-hover:bg-slate-800 group-hover:bg-opacity-40'>
                <ExternalLinkIcon className="w-20 place-self-center hidden h-2/3 group-hover:block"/>
